...
**Bundle-completeness gate (before the review gate).** After a topic's posts are bundled, confirm each is complete before going further — for `N in 1..N`: `research-verify-bundle --dir outputs/$D/$ACC/postN --account $ACC` (must exit 0). Any failure means that post's production did not finish (a re-dispatch or a re-run of the failed step is required); do not present or publish an incomplete post. This catches a silently dead subagent that never wrote its bundle.

**Front-card review gate (before publishing).** After all of a topic's posts are bundled and verified, and before ANY publishing, present each post to the operator for front-card approval: the `card_01.jpg` path, the headline, whether the **hero image** or the **motif fallback** was used (the `research-hero` exit code tells you — 0 = hero, non-zero = motif), and the `hero_image_prompt`. The operator responds per post:
- **approve** → keep as-is.
