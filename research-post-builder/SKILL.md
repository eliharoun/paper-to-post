...
Let **N = `max_posts_needed()`** for this topic (the greediest channel's `max_posts`; if it's `None`, N = every candidate that clears the bar). Load `references/scoring-rubric.md`. Score the top ~15–25 candidates 0–100 (apply the **scroll test** — what would a curious professional stop for). **Rank them and take the best N that clear `MIN_SCORE_TO_POST` (advisory `.env` knob, default 70 — honored by your judgment, not enforced by any script) and aren't already delivered.** Pick *distinct, non-redundant* papers (avoid two near-identical results — spread the interest).
...
### C. Per-post pipeline (steps 1–5, repeat for each selected post N)
...
**For a large run (N ≳ 3, or multiple topics), dispatch one subagent per post** rather than doing them all in your own context. Each post requires reading a full paper (often tens of thousands of tokens), and doing 10–15 in sequence yourself risks exhausting context mid-run. The posts are fully independent (separate `postN/` dirs), so give each subagent the exact steps 1–5 below for its one `postN`, and have it report back only a short status (headline, validation exit 0?, front card = hero or motif fallback?, screenshot yes/no, card count, issues) — never the post body. Then you verify all bundles and handle delivery. Steps per post:
...
3. **Validate (HARD GATE).** `research-validate --post outputs/$D/$ACC/postN/run/post.json --paper outputs/$D/$ACC/postN/run/selected_paper.json --account $ACC`. Exit 1 → read `errors[]`, fix `post.json` without adding facts, re-run (≤3 attempts). If it still fails, **drop this paper and promote the next-best candidate** from step B into `postN`. Never render while validation fails.
   ...
   **Pass on the first try — the recurring trip-ups (avoid these while writing, not on retry):**
   - **Banned hype terms** anywhere, including the caption: `proven`/`proves`, `cure`, `breakthrough`, `revolutionary`, `miracle`, `game-changer`, `guaranteed`, `100%`. Write "shown"/"demonstrated"/"suggests" from the start; don't smuggle "proven" into a limitation or caption line.
...
5. **Bundle.** `research-bundle --post outputs/$D/$ACC/postN/run/post.json --paper outputs/$D/$ACC/postN/run/selected_paper.json --assets-dir outputs/$D/$ACC/postN/assets --screenshot outputs/$D/$ACC/postN/run/paper_page.jpg --out outputs/$D/$ACC/postN --date $D` → inserts the screenshot second-to-last, renumbers, writes `caption.txt` + `post.json`, records the paper in the ledger.

### D. Deliver to each configured channel (config-driven)

**Front-card review gate (before publishing).** After all of a topic's posts are bundled and before ANY publishing, present each post to the operator for front-card approval: the `card_01.jpg` path, the headline, whether the **hero image** or the **motif fallback** was used (the `research-hero` exit code tells you — 0 = hero, non-zero = motif), and the `hero_image_prompt`. The operator responds per post:
- **approve** → keep as-is.
...
- **reject-to-motif** → rebuild the front card from the motif: `research-render --post outputs/$D/$ACC/postN/run/post.json --out outputs/$D/$ACC/postN/assets --account $ACC --motif-key "$D-$N" --start-index 1`, then re-bundle that post (step 5).
...
| Render cards | `research-render --post outputs/$D/$ACC/post$N/run/post.json --out outputs/$D/$ACC/post$N/assets --account $ACC --motif-key "$D-$N"` (add `--start-index 2` when the hero wrote `card_01.jpg`) |
