  ...
  "carousel_cards": [
    {
      ...
      "footer": "string — leave empty (\"\"); auto-generated at render time"
    }
  ],
...
- **`carousel_cards`** count must be between `brand.min_cards` (5) and `brand.max_cards` (7), numbered sequentially from 1. **Card 1 must be `card_type: "title"`** (headline in `heading`, empty `body`, empty `footer`) — its `heading` is the front-card thumbnail headline. **Footers are auto-generated at render time and must be left `""` on every card:** the pipeline computes them from the paper (title/hero card and closing source card = no footer; every content card between them = `Source · Category · Month D, YYYY`), so any footer you author is ignored. That `heading` is overlaid on the AI **hero image** when one is generated (see `hero-image-guide.md`), or on the branded **motif** backdrop as the fallback; either way the `heading` you write is the text that ships. Content cards use question `heading`s (see instagram-writing-guide). Include a `source` card. The paper first-page screenshot is inserted automatically before the source card at render time — you do not author a screenshot card.
